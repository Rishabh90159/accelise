      ...
      <section className="section">
        <div className="container grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            ].map(([title, text]) => <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm transition hover:border-[#8fb0ff] hover:shadow-md" key={title}><h3 className="font-extrabold text-[#0b1b3a]">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{text}</p></div>)}
          </div>
        </div>
      </section>
